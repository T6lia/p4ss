...
class Roles_RolesModel extends Model_Abstract
{
    ...
    public function createFromAdmin($data) {
        ...
        $query = "INSERT INTO {$this->_tablename} (id_appli, username_appli, parent_id, parent_username, nom, prenom,
                  mail, telephone, adresse, identite, pack, confirme_check, date_confirmation) 
                  VALUES (:id_appli, :username_appli, :parent_id, :parent_username, :nom, :prenom,
                  :mail, :telephone, :adresse, :identite, :pack, :confirme_check, :date_confirmation) ";
    ...
    public function createFirst($data) {
        ...
        $query = "INSERT INTO {$this->_tablename} (id_appli, username_appli, pack, parent_username, parent_id) 
                  VALUES (:id_appli, :username_appli, :nom_pack, :parent_username, :parent_id) ";
    ...
    /**
     * @return array
     * Count number of invalid user
     */
    public function countNotValid() {

        $query = "SELECT COUNT(username_appli) as not_valid FROM {$this->_tablename} 
                  WHERE confirme_check IS NULL ";
     ...
     * @param $user
     * @return mixed
     * Pour une tentative de connexion meme en etant non validee
     */
    public function connexInvalid($user) {

        $query = "SELECT confirme_check FROM {$this->_tablename} WHERE username_appli =:username ";
        $stmt = Database::prepare($query);
