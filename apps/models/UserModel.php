...
class UserModel extends Model_Abstract {
    ...
    public function getUserLogin($userName, $password) {
        ...
        $query .= "LEFT JOIN users_roles AS usrs_rls ON usrs.role_id = usrs_rls.id ";
        //$query .= "LEFT JOIN ref_sites AS site ON usrs.user_site_code = site.code ";
        //$query .= "LEFT JOIN ref_sites AS st ON usrs.user_site_code = st.code ";
        ...
        if (count($preselectUser) > 0) {
            foreach ($preselectUser as $key => $user) {
                //if ($password == Apps::decryptIt($user->password)) {
